...
public class Lexer implements ILexer {
    ...
    int next = 0;
    int currentToken = 0;
    boolean backSequenceChar = false;
    ...
    public char advance_stringToLexeme() {
        current++;
        //next character in source string
        if (source.charAt(current - 1) == '\"') {
            if (source.charAt(current - 2) == '\\') {
                backSequenceChar = false;
                return '"';
            } else {
                return source.charAt(current - 1);
            }
    ...
    public void stringToLexeme() {
        ...
        while (backSequenceChar && !isAtEnd()) {
            if (char_peek() == '\n') {
                line++;
                column = 0;
            }
            advance_stringToLexeme();
        }
        // Unterminated string.
        if (isAtEnd()) {
            return;
